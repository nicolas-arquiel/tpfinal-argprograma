...
export async function getMedicos() {
  try {
    const data = await sql`SELECT * FROM medicos`;
    console.log(data);
...
export async function getPacientes() {
  ...
  // Función para obtener un médico por ID
  export async function getMedicoById(medicoId) {
    try {
      const medico = await sql`SELECT * FROM medicos WHERE id = ${medicoId}`;
      ...
      throw new Error('Error al obtener el médico por ID');
    }
  }
