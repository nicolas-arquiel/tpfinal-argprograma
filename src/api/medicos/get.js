...
export async function getMedicos() {
  try {
    ...
    WHERE (medicos.obra_social_id IS NULL OR medicos.especialidad_id IS NULL)
    ...
    try {
      const data = await sql`
        SELECT 
          ...
          medicos.nombre AS label
        FROM medicos
        ${obraSocialId ? sql`INNER JOIN medicos_obras_sociales ON medicos.id = medicos_obras_sociales.medico_id` : sql``}
        WHERE 
          (${obraSocialId ? sql`medicos_obras_sociales.obra_social_id = ${obraSocialId}` : sql``})
          ${obraSocialId && especialidadId ? sql`AND` : sql``}
          (${especialidadId ? sql`medicos.especialidad_id = ${especialidadId}` : sql``})
          AND medicos.activo = true
        ORDER BY medicos.nombre;
      `;
      ...
      console.error('Error en la base de datos:', error);
      throw new Error('Error al obtener los médicos según obra social y especialidad');
    }
  }
